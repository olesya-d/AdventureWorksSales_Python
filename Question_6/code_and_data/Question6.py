...
sns.boxplot(data=df, x='AnnualRevenue', y='SquareFeet', ax=axes[0])

axes[0].set_title('Shop Size Distribution by Revenue', fontsize=16)
axes[0].set_xlabel('Annual Revenue(thousands)', fontsize=12)
axes[0].set_ylabel('Square Feet(thousands)', fontsize=12)
...
for i in range(len(stats_df)):
    ...
    axes[0].text(i, min, f'min: {int(min/1000)}', ha='center', va='top', fontsize=9)
    axes[0].text(i, max, f'max: {int(max/1000)}', ha='center', va='bottom', fontsize=9) 
...
sns.regplot(data=df, x='SquareFeet', y='AnnualRevenue',  line_kws={'color': 'red'}, ax=axes[1])

axes[1].set_title('Correlation between  Revenue and Shop Size', fontsize=16)
axes[1].set_ylabel('Annual Revenue(thousand)', fontsize=12)
axes[1].set_xlabel('Square Feet (thousands)', fontsize=12)
axes[1].set_xlim(5000, None)
...
ticks_y = rev_group
axes[1].set_yticks(ticks_y)
axes[1].set_yticklabels(labels_y)

# -------- Employee by Revenue boxplot--------

fig, axes = plt.subplots(1, 2, figsize=(16, 6)) 
sns.boxplot(data=df, x='AnnualRevenue', y='NumberEmployees',ax=axes[0])

axes[0].set_title('Employees Distribution by Revenue', fontsize=16)
axes[0].set_xlabel('Annual Revenue(thousands)', fontsize=12)
...
for i in range(len(stats_df)):
    min = stats_df.loc[i,'min']
    max = stats_df.loc[i,'max']
    axes[0].text(i, min, f'min: {int(min)}', ha='center', va='top', fontsize=9)
    axes[0].text(i, max, f'max: {int(max)}', ha='center', va='bottom', fontsize=9)
...
sns.regplot(data=df, x='NumberEmployees', y='AnnualRevenue', line_kws={'color': 'red'},ax=axes[1])
axes[1].set_title('Correlation between  Revenue and Number of Employees', fontsize=16)
axes[1].set_ylabel('Annual Revenue(thousand)', fontsize=12)
axes[1].set_xlabel('NumberEmployees', fontsize=12)
